...
def create_search_embed(query, data, page_index=1, show_details=False):
    ...
    for i in range((page_index - 1) * SEARCH_RESULTS_LIMIT_PER_PAGE, (page_index - 1) * SEARCH_RESULTS_LIMIT_PER_PAGE + limit_per_page):
        ...
        embed_value_details_links = f"[Details](https://jisho.org/word/{japanese_word})\n[Kanji details](https://jisho.org/search/{japanese_word}%20%23kanji)"
        embed_value_intro = '\n'.join(filter(None, [
            f"{japanese_word} {'[{0}]'.format(reading) if reading is not None else ''}", additional_info]))
        embed_value_results = '\n'.join(english_definitions)
        embed_value = f"{embed_value_details_links}```{embed_value_intro}\n{embed_value_results}```"
